import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:plebshub_ui/plebshub_ui.dart';

import '../providers/reaction_provider.dart';

/// A button for liking/reacting to a post.
///
/// Shows a heart icon that toggles between outline (not liked) and filled (liked).
/// Displays the reaction count next to the icon.
/// Uses optimistic UI updates for immediate feedback.
///
/// Example:
/// ```dart
/// LikeButton(
///   eventId: post.id,
///   authorPubkey: post.author.pubkey,
/// )
/// ```
class LikeButton extends ConsumerWidget {
  const LikeButton({
    super.key,
    required this.eventId,
    required this.authorPubkey,
  });

  /// The ID of the event/post to like
  final String eventId;

  /// The public key of the event author (for NIP-25 'p' tag)
  final String authorPubkey;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final reactionState = ref.watch(reactionProvider);
    final hasLiked = reactionState.hasUserLiked(eventId);
    final count = reactionState.getReactionCount(eventId);
    final isPending = reactionState.pendingLikes.contains(eventId);

    return InkWell(
      onTap: () {
        ref.read(reactionProvider.notifier).toggleLike(
              eventId: eventId,
              authorPubkey: authorPubkey,
            );
      },
      borderRadius: BorderRadius.circular(8),
      child: Padding(
        padding: const EdgeInsets.all(8),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            // Animated heart icon
            AnimatedSwitcher(
              duration: const Duration(milliseconds: 200),
              transitionBuilder: (child, animation) {
                return ScaleTransition(
                  scale: animation,
                  child: child,
                );
              },
              child: Icon(
                hasLiked ? Icons.favorite : Icons.favorite_border,
                key: ValueKey(hasLiked),
                size: 20,
                color: hasLiked
                    ? Colors.red
                    : isPending
                        ? Colors.red.withValues(alpha: 0.5)
                        : AppColors.textSecondary,
              ),
            ),
            const SizedBox(width: 4),
            // Reaction count
            Text(
              _formatCount(count),
              style: AppTypography.labelMedium.copyWith(
                color: hasLiked ? Colors.red : null,
              ),
            ),
          ],
        ),
      ),
    );
  }

  /// Format count for display.
  ///
  /// Shows abbreviated numbers for large counts:
  /// - < 1000: show as-is (e.g., "42")
  /// - >= 1000: show as K (e.g., "1.2K")
  /// - >= 1000000: show as M (e.g., "1.5M")
  String _formatCount(int count) {
    if (count < 1000) {
      return count.toString();
    } else if (count < 1000000) {
      final k = count / 1000;
      return '${k.toStringAsFixed(k < 10 ? 1 : 0)}K';
    } else {
      final m = count / 1000000;
      return '${m.toStringAsFixed(m < 10 ? 1 : 0)}M';
    }
  }
}
